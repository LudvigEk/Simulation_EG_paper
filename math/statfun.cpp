...
void Stat_Normalize_double(double *aData, int nSize)
{
							 ...
							 const float s1, const float ss1, int n1)
{
	...
	/*
	if (n0>1)
		return float(Stat_GetVariance(s0, ss0, n0));
	else
		return float(Stat_GetVariance(s1, ss1, n1));
	*/
}
...
void Quicksort_Indexed_Ascending(Stat_SortItem *pLeft, Stat_SortItem *pRight)
{
	if (pRight<=pLeft)
		return;

	Stat_SortItem *p_i= pLeft-1;
	Stat_SortItem *p_j= pRight+1;
	...
	do 
	{
		while ((++p_i)->m_fValue<v);
		while ((--p_j)->m_fValue>v);
	...
	*p_j= tmp;
	...
	Quicksort_Indexed_Ascending(pLeft, p_j);
	Quicksort_Indexed_Ascending(p_j+1, pRight);
}

void Quicksort_Indexed_Descending(Stat_SortItem *pLeft, Stat_SortItem *pRight)
{
	if (pRight<=pLeft)
		return;
	...
	*p_j= tmp;
	...
	Quicksort_Indexed_Descending(pLeft, p_j);
	Quicksort_Indexed_Descending(p_j+1, pRight);
}

void Stat_QuicksortIndexed(Stat_SortItem *aSI, int nSize, bool bAscending)
{
	if (bAscending)
		Quicksort_Indexed_Ascending(aSI, aSI + (nSize - 1));
	else
		Quicksort_Indexed_Descending(aSI, aSI + (nSize - 1));
}
...
bool Stat_HistConvolve(const CVector<double> &h0, float min0, float max0, 
					   const CVector<double> &h1, float min1, float max1, 
					   CVector<double> &h_out, float &min_out, float &max_out)
{
	...
	{
		const float x0= float(i0)/nm1*(max0-min0)+min0;
		const double n0= h0[i0];
		for (int i1=0;i1<n;i1++)
		{
			float x1= float(i1)/nm1*(max1-min1)+min1;
			float x_out= x0+x1;
...
double Stat_GetInnerProduct(double *aData0, double *aData1, int n)
{
	double s=0;
	for (int i=n;--i>=0;)
		s += aData0[i]*aData1[i];
...
bool Stat_NormalizeMatrix(CMatrix<float> &m, int mode)
{
	...
	CVector<Stat_SortItem> vSI(n);
	Stat_SortItem *pSI= vSI.GetBuffer();
	for (int c=0;c<m.GetWidth();c++)
	{
		for (int i=0;i<n;i++)
		{
			pSI[i].m_fValue= m.GetAt(i,c);
			pSI[i].m_nIndex= i;
		}
		
		Stat_QuicksortIndexed(pSI, vSI.GetSize(), true);
