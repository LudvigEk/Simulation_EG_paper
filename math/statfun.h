...
void Stat_QuicksortIndexed_double(Stat_SortItem_double *aSI, int nSize, bool Ascending);
...
inline float Stat_GetStandardError(const float s, const float ssqr, const int nSize)
{ return (float)sqrt((ssqr-s*s/nSize)/(nSize-1)); }
inline float Stat_GetStandardError(const float *aData, const int nSize)
{ return (float) sqrt(Stat_GetVariance(aData, nSize)); }

float Stat_GetCohen(float m0, float v0, float n0, float m1, float v1, float n1);
