				  ...
				  CVector<CString> &vGeneSetName, CVector< CVector<int> > &vGeneSet, int c)
{
	...
	for (int r0=nAnnotRows_sco;r0<idx_sco.GetRowCount();r0++)
	{
		...
		for (int i=0;r1<0 && i<vID.GetSize();i++)
			r1= Table_FindInIndex(vID[i], vSI_set);
	...
	Table_SortItems(vSI, true, false);
		...
		if (!sSetName.IsEmpty())
		{
			...
			for (int c=1;c<C;c++)
			{
				...
				if (false)
				{
					...
					i= Table_FindAnnotRow(idx_sco, Table_GetAt(idx_set, r, c), idx_sco.GetRowCount()); // TODO: This is SLOW! -- replace
				}
				else
				{
...
bool Sets_CheckIdentifiers(const CTableIndex &idx_sco, int nAnnotRows_sco, int nKeyCol_sco)
{
	...
	CVector<Table_SortItem> vSI(idx_sco.GetRowCount()-nAnnotRows_sco);
	for (int r=nAnnotRows_sco;r<idx_sco.GetRowCount();r++)
	{
		Table_SortItem si;
		si.m_sKey= Table_GetAt(idx_sco, r, nKeyCol_sco);
				  ...
				  const CVector<CString> &vGeneSetName, const CVector< CVector<int> > &vGeneSet)
{
	...
	for (int i=0;bOk && i<vGeneSetName.GetSize();i++)
	{
		...
		for (int j=0;j<vGeneSet[i].GetSize();j++)
			fprintf(pf, "\t%s", Table_GetAt(idx_sco,  nAnnotRows_sco + vGeneSet[i][j], nKeyCol_sco));
		fprintf(pf, "\n");
	}
...
void Sets_CreateKey(const CTableIndex &idx, int nAnnotRows, int nKeyCol, CVector<Table_SortItem> &vSI)
{
	...
	Table_SortItem *pSI= vSI.GetBuffer(vSI.GetSize());
...
bool Sets_Init_sco(const char *aFilename,
				   ...
				   CString sScoreCol_sco, int &nScoreCol_sco, CMatrix<float> &m_Data_sco)
{
	...
	printf("scan floats\n");
	if (!Table_ScanFloatMatrix(idx_sco, nAnnotRows_sco, nAnnotCols_sco, idx_sco.GetRowCount()-1, idx_sco.GetMinColCount()-1, m_Data_sco, NULL))
	{
		ReportError("Illegal floating point format", aFilename);
		return false;
	}
						...
						int &nKeyCol_set, CVector<Table_SortItem> &vKey_set)
{
	...
	// Check format
	bool bOk= false;
	if (idx_set.GetMinColCount()<2)
		ReportError("Gene set table must contain at least two columns", aFilename);
	else
		bOk= true;
