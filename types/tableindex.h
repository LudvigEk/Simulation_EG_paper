...
public:
	...
	bool ScanTDS(const char *ach, PARSECALLBACK pCallback=NULL); //Creates the CTableIndex from tab-delim file 
	bool ScanCSV(const char *ach, PARSECALLBACK pCallback=NULL); //Creates the CTableIndex from comma-sep file 

	bool CreateSubset(CMatrix<std::pair<const char *, const char *>> matrix, const CTableIndex &source); //Creates new CTableIndex from another //by Ludde

	bool IsEmpty() const;
	bool IsRectangular() const;

	// No longer wanted with MakeRectangular around.
	// const char *GetLastPointerPast(int nRow, int nCol) const;
